...
*   a Rayleigh distribution and a Pareto distribution.
...
import elvira.tools.statistics.plot.Plot;

import elvira.tools.statistics.plot.PlotGraph;

import elvira.tools.statistics.math.Matrix;
...
public class Regression{
    ...
    private int nData=0;        		// number of y data points

    private int nXarrays=0;     		// number of x arrays
    ...
    private Vector  best = new Vector();    // best esitimates vector of the unknown parameters

    private Vector  bestSd = new Vector(); 	// standard deviation estimates of the best estimates of the unknown parameters
    ...
    private Vector penalties = new Vector();// penalty parameter index, bleow or above constraint flag, constraint repeated
    ...
    private void generalLinear(double[][] xd){
	        ...
	        for (int i=0; i<this.nTerms; ++i){
	    	    ...
	    	    for (int j=0; j<this.nTerms; ++j){
	    		    ...
	    		    for (int k=0; k<this.nData; ++k){
                        ...
                        sum += 2.0*xd[i][k]*xd[j][k]/Fmath.square(sde);
        ...
        this.multCorrelCoeff(this.xData, this.yData, this.weight);
    ...
    private void nelderMead(RegressionFunction g, double[] start, double[] step, double fTol, int nMax){
	    ...
	    yy[nnp-1]=this.sumSquares(g, start);
	    ...
	    while(test){
	        ...
	        if(ystar < ylo){
	            ...
	            y2star=this.sumSquares(g, p2star);
	...
	private double sumSquares(RegressionFunction g, double[] x){
            ...
            for(int i=0; i<this.nConstraints; i++){
                ...
                if(this.penaltyCheck[i]==-1){
...

                    {
                        ...
                        ss = this.lastSSnoConstraint + this.penaltyWeight*Fmath.square(param[k]-constraints[i]);
        ...
        if(test){
            ...
            for(int i=0; i<this.nData; i++){
                ...
                for(int j=0; j<nXarrays; j++)xd[j]=this.xData[j][i];
                    ...
                    ss += Fmath.square((this.yData[i] - g.function(param, xd))/this.weight[i]);
	...
	// add a constraint boundary for the non-linear regression
	...
	public void addConstraint(int paramIndex, int conDir, double constraint){
	    ...
	    if(!this.penalties.isEmpty()){
		    ...
		    // remove constraints if new method

		    Integer itemp = null;

		    itemp = (Integer)this.penalties.elementAt(0);

		    if(itemp.intValue()!=this.lastMethod){
    ...
    private int linearStats(RegressionFunction g){
	    ...
	    for (int i=0;i<np; ++i){
		    ...
		    this.grad[i][0]=(this.fMin-f1)/(this.delta*hold0);

		    this.grad[i][1]=(f2-this.fMin)/(this.delta*hold0);
	    ...
	    for(int i=0; i<this.nData; i++){
	        ...
	        this.yCalc[i] = g.function(pmin, xd);
	        ...
	        if (this.invertFlag){
		        ...
		        for (int i=0; i<np; ++i){
			        ...
			        for (int j=i; j<np;++j){
                        ...
                        if(hold0==0.0D)hold1=this.step[j];

				        this.covar[i][j] = stat[i][j]*red*hold0*hold1;
	...
	public void print(String filename){
        ...
        switch(this.lastMethod){
	        ...
	        case 1: fout.println("Polynomial Fitting Linear Regression");
	...
	private void nonLinearPrint(FileOutput fout){
        ...
        if(!this.scaleFlag){
            ...
            fout.println("The ordinate scaling factor [yscale, Ao] has been set equal to unity");
    ...
    public void plotYY(String title){
        ...
        for(int i=0; i<nData; i++){
            ...
            data[0][i]=this.yData[i];

            data[1][i]=this.yCalc[i];
    ...
    public String setTitle(String title){
        ...
        switch(this.lastMethod){
	        ...
	        case 1: title1 = "Linear(polynomial) regression: "+title;
    ...
    private int plotXY(RegressionFunction g, String title){
        ...
        int flag = this.plotXYnonlinear(g, title);
    ...
    private int plotXYlinear(String title){
        ...
        // Add axis legend titles

        if(!this.legendCheck)this.addLegends();
        ...
        if(npoints<this.nData)npoints=this.nData;

        if(this.lastMethod==11 || this.lastMethod==12 || this.lastMethod==21)npoints=this.nData;
        ...
        if(this.nTerms==0){
	        ...
	        case 11: title1 = "No regression: Minimum Order Statistic Standard Gumbel (y = exp(x)exp(-exp(x))): "+title;
	        ...
	        case 12: title1 = "No regression:  Maximum Order Statistic Standard Gumbel (y = exp(-x)exp(-exp(-x))): "+title;
	        ...
	        case 21: title1 = "No regression:  Standard Exponential (y = exp(-x)): "+title;
	        ...
	        case 0: title1 = "Linear regression  (y = a + b.x): "+title;
	        ...
	        case 2: title1 = "Linear regression  (y = a.x): "+title;
	        ...
	        case 11: title1 = "Linear regression: Minimum Order Statistic Standard Gumbel (y = a.z where z = exp(x)exp(-exp(x))): "+title;
	        ...
	        case 12: title1 = "Linear regression:  Maximum Order Statistic Standard Gumbel (y = a.z where z=exp(-x)exp(-exp(-x))): "+title;
    ...
    public int plotXYnonlinear(RegressionFunction g, String title){
	    ...
	    if(this.nXarrays>1){
            ...
            if(lastMethod==6)npoints=this.nData;
	...
	public boolean getPosVarCheck(){
        ...
        public double getResidualVariance(){
            ...
            return Stat.variance(residuals);
        }

        //Get the Residual StardardDeviation
        public double getResidualStandardDeviation(){
            double [] residuals=this.getResiduals();
            return Stat.standardDeviation(residuals);
	...
	private void fitPoisson(int plotFlag){
        ...
        if(this.scaleFlag)start[1] = peak/(Math.exp(mean*Math.log(mean)-Stat.logFactorial(mean))*Math.exp(-mean));

        step[0] = 0.1D*start[0];

        if(step[0]==0.0D){

            Vector ret0 = Regression.dataSign(xData[0]);
        ...
        f.scale = this.scaleFlag;

        this.nelderMead(f, start, step, this.fTol, this.nMax);
	...
	private void fitGaussian(int plotFlag){
	    ...
	    Vector retY = Regression.dataSign(yData);
    ...
    private void multCorrelCoeff(double[][] xx, double[] yy, double[] ww){
        ...
        temp0 = (1.0D/((double)this.nData - 1.0D))/(sumRecipW/(double)this.nData);



        // weighted means

        double[] mj = new double[this.nXarrays];
        ...
        // calculate sample variances x-x

        double[] s2jj = new double[this.nTerms];
        ...
        for(int i=0; i<this.nXarrays; i++){
            ...
            for(int k=0; k<this.nData; k++){
                ...
                temp1 += Fmath.square((xx[i][k]-mj[i])/ww[k]);
        ...
        // calculate sample variances y-x

        double[] s2jy = new double[this.nTerms];
        ...
        for(int i=0; i<this.nXarrays; i++){
            ...
            for(int k=0; k<this.nData; k++){
                    ...
                    temp1 += (xx[i][k]-mj[i])*(yy[k]-my)/Fmath.square(ww[k]);
        ...
        // calculate sample variances y-y
        ...
        // calculate multiple corelation coefficient

        this.sampleR = 0.0D;

        for(int i=0; i<this.nXarrays; i++){

            this.sampleR += Fmath.square(s2jy[i])/(s2yy*s2jj[i]);

        }

        this.sampleR = Math.sqrt(this.sampleR);

        if(this.nXarrays>1){

            temp1 = Fmath.square(this.sampleR);

            this.multipleF = temp1*(this.nData-this.nXarrays)/((1.0D-temp1)*(this.nXarrays-1));
    ...
    // check data arrays for sign, max, min and peak
 	...
 	private static Vector dataSign(double[] data){
...
/*
    ...
    private void fitFrechetWeibull(int allTest, int typeFlag){
	    ...
	    Vector retX = Regression.dataSign(this.xData[0]);
        ...
        switch(typeFlag){
            ...
            case 0: start[0] = ests[0];         //mu
                    ...
                    if(this.scaleFlag)start[3] = 1.0/yScale;      //y axis scaling factor
            ...
            case 1: start[0] = ests[0];         //sigma
                    ...
                    if(this.scaleFlag)start[2] = 1.0/yScale;      //y axis scaling factor
            ...
            case 2: start[0] = ests[0];         //gamma
                    ...
                    if(this.scaleFlag)start[1] = 1.0/yScale;      //y axis scaling factor
        ...
        if(this.frechetWeibull){
            ...
            ff.scale = this.scaleFlag;

            this.nelderMead(ff, start, step, this.fTol, this.nMax);
...
*/
...

	{
	    ...
	    Vector retX = Regression.dataSign(xData[0]);
        ...
        switch(typeFlag){
            ...
            case 1:
                    ...
                    if(this.scaleFlag)start[2] = yPeak*start[1]*Math.exp(1);      //y axis scaling factor
            ...
            case 3:
                    ...
                    if(this.scaleFlag)start[1] = yPeak*start[0]*Math.exp(1);      //y axis scaling factor
...

	{
        ...
        switch(typeFlag){
            ...
            case 0: start[0] = xMin*0.9;    //mu
                    ...
                    if(this.scaleFlag)start[2] = yPeak*sigma; //y axis scaling factor
            ...
            case 1: start[0] = sigma;       //sigma
                    ...
                    if(this.scaleFlag)start[1] = yPeak*sigma; //y axis scaling factor
