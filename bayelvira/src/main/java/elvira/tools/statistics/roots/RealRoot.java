...
public class RealRoot{
    ...
    private static double tol = 1e-9;           // tolerance in determining convergence upon a root
    private static double tolClass = 1e-9;      // class default value for tol
    private static int iterMax = 100;           // maximum number of iterations allowed in root search
    private static int iterN = 0;               // number of iterations taken in root search

    // Reset the default tolerance
    public void setTolerance(double tolerance){
        RealRoot.tolClass=tolerance;
    }

    // Get the default tolerance
    public double getTolerance(){
        return RealRoot.tolClass;
    }

    // Reset the maximum iterations allowed
    public void setIterMax(int imax){
        RealRoot.iterMax=imax;
    }

    // Get the maximum iterations allowed
    public int getIterMax(){
        return RealRoot.iterMax;
    }

    // Get the number of iterations taken
    public int getIterN(){
        return RealRoot.iterN;
    }

    // Combined bisection and Newton Raphson method
    // required accuracy supplied
   	public static double brent(RealRootFunction g, double lower, double upper, double acc){
        RealRoot.tol=acc;
        double root = brent(g, lower, upper);
        RealRoot.tol=RealRoot.tolClass;
        return root;
    }

    // Combined bisection and Newton Raphson method
    // default accuracy used
   	public static double brent(RealRootFunction g, double lower, double upper){
	    // check upper>lower
	    ...
	    while(testConv){
	        ...
	        if(fm==0.0D || Math.abs(diff)<RealRoot.tol){
	            testConv=false;
	            if(fm==0.0D){
	                root=lastMid;
	            }
	            else{
	                if(Math.abs(diff)<RealRoot.tol)root=mid;
	            }
	        }
	        else{
            ...
            RealRoot.iterN++;
            if(RealRoot.iterN>RealRoot.iterMax){
                System.out.println("RealRoot.brent: maximum number of iterations exceeded - root at this point returned");
                System.out.println("Last mid-point difference = "+diff+", tolerance = " + RealRoot.tol);
                root = mid;
                testConv = false;
            }
        }
    ...
    // bisection method
    // accuracy supplied
	public static double bisect(RealRootFunction g, double lower, double upper, double acc){
	    RealRoot.tol=acc;
    ...
    // bisection method
    // default accuracy used
	public static double bisect(RealRootFunction g, double lower, double upper){
	    // check upper>lower
	    if(upper==lower)throw new IllegalArgumentException("upper cannot equal lower");
	    if(upper<lower){
            double temp = upper;
	    ...
	    double root = -1.235e-200;  // variable to hold the returned root
        boolean testConv = true;    // convergence test: becomes false on convergence
        RealRoot.iterN = 0;         // number of iterations
    ...
    // accuracy suppled
	public static double falsePosition(RealRootFunction g, double lower, double upper, double acc){
        RealRoot.tol=acc;
    ...
    // false position  method
    // default accuracy used
	public static double falsePosition(RealRootFunction g, double lower, double upper){
	    // check upper>lower
	    if(upper==lower)throw new IllegalArgumentException("upper cannot equal lower");
	    if(upper<lower){
 	        double temp = upper;
    ...
    // Combined bisection and Newton Raphson method
    // accuracy supplied
   	public static double bisectNewtonRaphson(RealRootDerivFunction g, double lower, double upper, double acc){
        RealRoot.tol=acc;
    ...
    // Combined bisection and Newton Raphson method
    // default accuracy used
   	public static double bisectNewtonRaphson(RealRootDerivFunction g, double lower, double upper){
	    // check upper>lower
	    if(upper==lower)throw new IllegalArgumentException("upper cannot equal lower");
    ...
    // accuracy supplied
	public static double newtonRaphson(RealRootDerivFunction g, double x, double acc){
        RealRoot.tol=acc;
    ...
    // Newton Raphson method
    // default accuracy used
	public static double newtonRaphson(RealRootDerivFunction g, double x){
	    double root = -1.235e-200;  // variable to hold the returned root
        boolean testConv = true;    // convergence test: becomes false on convergence
        RealRoot.iterN = 0;         // number of iterations
        ...
        while(testConv){
            diff = f[0]/f[1];
            if(f[0]==0.0D || Math.abs(diff)<RealRoot.tol){
                root = x;
                testConv=false;
            }
            else{
