...
public class Matrix{
	...
	// Construct matrix with a reference to the 2D matrix and permutation index of an existing ComplexMatrix bb.
    	...
    	public static Matrix scalarMatrix(int nrow, double diagconst){
        	Matrix u = new Matrix(nrow, nrow);
        	double[][] uarray = u.getArrayPointer();
        	for(int i=0; i<nrow; i++){
    	...
    	public Matrix getSubMatrix(int i, int j, int k, int l){
        	...
        	double[][] sarray = this.getArrayPointer();
        	for(int p=0; p<n; p++){
            		for(int q=0; q<m; p++){
   	...
   	// col = array of column indices
    	...
    	public static Matrix copy(Matrix a){
        	int nr = a.getNrow();
        	int nc = a.getNcol();
        	double[][] aarray = a.getArrayPointer();
        	Matrix b = new Matrix(nr,nc);
        	b.nrow = nr;
        	b.ncol = nc;
        	double[][] barray = b.getArrayPointer();
        	for(int i=0; i<nr; i++){
            		for(int j=0; j<nc; j++){
    	...
    	public Matrix plus(Matrix bmat){
        	...
        	double[][] carray = cmat.getArrayPointer();
        	for(int i=0; i<nr; i++){
            		for(int j=0; j<nc; j++){
    	...
    	// MULTIPLICATION
    	// Multiply this  matrix by a matrix.   [non-static method]
    	...
    	// Multiply this matrix by a constant [non-static method]
    	// This matrix remains unaltered
   	...
   	// Multiply a matrix by a constant [static method]
    	...
    	// INVERSE
    	// Inverse of a square matrix [non-static method]
    	public Matrix inverse(){
        	...
        	double[][] invarray = invmat.getArrayPointer();
        	Matrix ludmat;
    	...
    	public Matrix transpose(){
        	...
        	double[][] tarray = tmat.getArrayPointer();
    	...
    	// OPPOSITE
    	// Opposite of a matrix [non-static method]
    	...
    	// TRACE
    	// Trace of a  matrix [non-static method]
    	public double trace(){
        	double trac = 0.0D;
    	...
    	// DETERMINANT
    	//  Returns the determinant of a square matrix [non-static method]
    	...
    	// Returns the log(determinant) of a square matrix [non-static method].
	    ...
	    public Matrix luDecomp(){
	    	...
	    	double[][] ludarray = ludmat.getArrayPointer();
