...
public class ComplexPoly{
        ...
        public static ComplexPoly rootsToPoly(Complex[] roots){
            ...
            ComplexPoly poly = new ComplexPoly(pdeg);
            poly.resetCoeff(0, Complex.plusOne());
            ComplexPoly cpoly = poly.copy();
            ComplexPoly croot = new ComplexPoly(1);

            for(int i=0; i<pdeg; i++){
                    croot.resetCoeff(0, Complex.plusOne());
                    croot.resetCoeff(1, roots[i].times(-1.0D));
                    for(int j=0; j<pdeg; j++)poly.coeff[j] = cpoly.coeff[j].times(croot.coeff[0]);
                    for(int j=1; j<pdeg; j++)poly.coeff[j] = poly.coeff[j].plus(cpoly.coeff[j-1].times(croot.coeff[1]));
                    cpoly = poly.copy();
            }
            return poly;
        }
        ...
        // Return a copy of this ComplexPoly [non-static]
