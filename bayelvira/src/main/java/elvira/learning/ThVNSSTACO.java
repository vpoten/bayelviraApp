...
public class ThVNSSTACO implements Runnable {
    ...
    public ThVNSSTACO(Double fero[][],double rho,NodeList vars, DataBaseCases cases,Metrics met,Graph maxBnet
		      ,double maxFitness,double globalMaxFitness,int maxNb,int nProc, int nIndiv,
		      Random generator){
	...
	this.feromone = fero;
	this.rho = rho;
	this.globalMaxFitness = globalMaxFitness;
	...
	while(!stop){ 
		...
		maxBnetFitness = fitness;
		maxBnet = (Graph) currentBnet.duplicate();
		updateFeromone(maxBnetFitness,maxBnet,globalMaxFitness);
		nb = 5;
    ...
    private void updateFeromone(double maxBnetFitness,Graph maxBnet,double globalMaxFitness){
	double val,fit;
	int i,j;
	if(maxBnetFitness > globalMaxFitness){
	    fit = maxBnetFitness;
	    for(i=0 ; i < variables.size(); i++){
		Node nodei = variables.elementAt(i);
		for(j=0; j < variables.size();j++){
		    Node nodej = variables.elementAt(j);
		    Link link = maxBnet.getLinkList().getLinks(nodei.getName(),nodej.getName());
		    //NodeList vars = new NodeList();
		    //vars.insertNode(nodej);
		    //nodej = maxBnet.getNodeList().getNode(nodej.getName());
