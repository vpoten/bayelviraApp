...
public class ThVNSSTPC implements Runnable {
    ...
    int nProc;
    Random generator;
    Hashtable[] forbiddenParents;

    public ThVNSSTPC(){

    }

    public ThVNSSTPC(NodeList vars, DataBaseCases cases,Metrics met,Graph maxBnet
	...
	this.dag = new Graph(0);
        ...
        this.forbiddenParents = new Hashtable[variables.size()];
	for(i=0; i< variables.size(); i++){
	    try{
		dag.addNode(variables.elementAt(i));
	    }catch(InvalidEditException iee){};
            forbiddenParents[i] = new Hashtable();
	}
	this.cases = cases;
	this.metric = met;
	this.maxNb = maxNb;
    ...
    /**
     * This methods implements an optimal H_C(DAGS) algorithm.
     */
	...
	boolean OkToProceed,stop,lastIteration;
	Link newLink,link;
	...
	while(!stop){ 
		...
		op = maxScore(vlink,vvars,currentBnet,lastIteration);
		...
		if(op!=-1){
			...
			if(fitnessNew > fitness){
				...
				System.out.println("F["+nProc+","+indiv+"]:"+f+"   "+metric.getTotalStEval());
                            ...
                            if(!lastIteration) { 
                                    lastIteration = true;
                                    System.out.println("Reinicio de padres prohibidos");
                            }
    ...
    private int maxScore(Vector vlinkR, Vector vvarsR,Bnet current,boolean lastIteration){
	...
	for(i=0; i<current.getNodeList().size(); i++){
		...
		if(i!=j){
			...
			val = valNewj - valOldj;
                        if(val > 0){
                           forbiddenParents[j].put(nodei.getName(),nodei);
                           forbiddenParents[i].put(nodej.getName(),nodej);
                        }
			...
			if(acc.indexOf(link.getHead()) == -1){
				...
				varsR.join(vars);
                      ...
                      FiniteStates pp = (FiniteStates) 
                         forbiddenParents[j].get(nodei.getName());
                      if( (lastIteration) || (pp == null)){
                        Vector acc = new Vector();
	                acc = current.directedDescendants(nodej);
	                if(acc.indexOf(nodei) == -1){
			    numIndEval++;
