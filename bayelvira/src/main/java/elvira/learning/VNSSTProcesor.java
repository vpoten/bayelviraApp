...
public class VNSSTProcesor implements Runnable {
    ...
    public VNSSTProcesor(int nproc,NodeList vars,DataBaseCases data,Metrics met
			 ,int popSize,int maxNb,Random generator,double evalIni){
	...
	ThVNSST[] thvnsst = new ThVNSST[popSize];
	...
	iteration++;
	for(i = 0 ; i< popSize ; i++){
	    thvnsst[i] = new ThVNSST(variables,cases, metric,
                             popDags[i],popFitness[i],maxNb,numberProc,i,generator);
