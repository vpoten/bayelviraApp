 ...
 *
 * @version $Revision: 1.3 $, $Date: 2007/03/26 17:03:11 $
...
*/
           if (cutPoints.size()>1){
              if (((Double)cutPoints.elementAt(cont)).doubleValue()<=data[i+2]){// && ((Double)cutPoints.firstElement()).doubleValue()<x1){
...
//          System.out.println("Drawing Data Markers!");
   ...
   */
      ...
      public void setCutPoints(Vector v){
          
            cutPoints=(Vector)v.clone();
      }

      public void fillArea(Graphics g, Rectangle w, double init, double end){
...
**        first and grab the next point
*/
           if (data[i]>init && data[i]<end){
               xPoints[0]=x0;
               yPoints[0]=w.y+w.height;
